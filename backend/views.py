...
class MainPage(APIView):
    def first_page(request):
        return HttpResponse("API Сервис заказа товаров")


class RegisterUser(APIView):
    '''
    Регистрация покупателя методом POST
    '''
    ...
    def post(self, request, *args, **kwargs):
        ...
        if items:
            try:
                ...
                basket, _ = Order.objects.get_or_create(user_id=request.user.id, status='cart')
                objects_created = 0
...
class OrderView(APIView):
    '''
    Класс для получения и размещения заказов пользователем
    '''
    permission_classes = [IsAuthenticated]
